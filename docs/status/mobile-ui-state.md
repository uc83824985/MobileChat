...
- Debug mode exposes a per-message **朗读** action. The first pass calls the local Qwen3-TTS plain HTTP wrapper at `POST http://127.0.0.1:8765/speak` with `text`, `mode: "replace"`, and `meta`, so a later read request interrupts current playback and replaces pending speech. This is intentionally PC-local and not persisted yet; endpoint and voice options remain future settings work.
