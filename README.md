...
- 区分聊天助手与功能助手；聊天助手引用可复用上下文 Profile，上下文总结与后续上下文压缩分别引用专用功能助手。
...
当前会话状态、上下文总结/压缩、中转站探测和跨设备迁移决策见[会话状态、上下文压缩与数据迁移](docs/architecture/conversation-state-and-portability.md)。
...
- 设置页支持独立编辑 API Profile、模型列表和上下文 Profile；模型配置区会展示当前 Profile 的完整模型清单。助手只引用已有模型，并配置允许模型列表、默认模型和上下文 Profile。
