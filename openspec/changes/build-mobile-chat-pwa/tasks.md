...
- 2026-07-14: Added a persistent API-key reveal control, empty descriptions for newly created profiles/models, top/bottom floating scroll toggle behavior, and documented the minimal manual/automatic context-summary path.
...
- 2026-07-17: Consolidated context reduction around rolling context summaries only. Context Profiles now own `summaryMaxChars`; model records no longer expose context-window or summary-size policy. Summary output is locally budget-checked, may get one rewrite attempt, and otherwise preserves the prior valid summary.
- 2026-07-17: Added first-stage image input and cache behavior: image button/file picker, desktop paste, labeled `[图片N]` references, message/draft previews, image cache cleanup, missing-cache placeholders, and request-time text fallbacks when image blobs are unavailable.
- 2026-07-17: Added the model-probing workbench and compact settings refinements: probe terminology, structured version/suffix rule editing, current-connection execution, success-only create-model results, header-level enable/delete actions, object-specific delete labels, silent autosave, and removal of count-only settings cards.
- 2026-07-18: Added the Android WebView packaging route. The default mobile deploy now builds a fixed-package APK (`com.uc83824985.mobilechat`) using a stable `https://appassets.androidplatform.net/app/index.html` WebView origin, stable local signing key, DOM Storage/IndexedDB support, and Android file chooser forwarding for image inputs. Legacy local-file deployment remains available only as an explicit compatibility mode.
- 2026-07-18: Added the Android-only **沉浸显示（Android）** setting. The value persists with app settings, is mirrored for startup preference restore, and is applied through an optional WebView JavaScript bridge so the APK can hide system bars and draw into short-edge cutout areas while desktop/browser launches keep normal window chrome.
- The full task checklist remains open where the spec still requires dirty-record repositories, endpoint validation, pricing metadata, utility assistant references, summary validation, merge import, complete context projection, provider-specific hosted tool variants, multimodal content sending, and full streaming event/error coverage beyond the current text-delta path.

## 2. Domain model and local persistence

- [ ] 2.1 Define versioned TypeScript domain types for API profiles, models, model pricing metadata, chat/utility assistants, bindings, conversations, messages, content parts, blobs, drafts, context summaries, display summaries, context budget reports, pre-send cache estimates, normalized usage stats, and settings
- [ ] 2.2 Create the `MobileChatDB` IndexedDB database schema, including context-summary storage, typed repository interfaces, transactions, and store indexes
...
- [ ] 3.2 Implement assistant create, edit, enable, disable, and delete operations with chat/utility kinds, built-in utility feature references, nested model bindings, and default-binding enforcement
- [ ] 3.3 Implement binding resolution and invalid-reference reporting without deleting historical or disabled configuration
- [ ] 3.4 Build the settings pages for API-profile details, model lists, optional pricing metadata, assistant kind, identity, prompt, initial message, utility feature references, and model bindings
...
- [ ] 4.2 Implement the initial OpenAI-compatible Responses API request builder using direct browser fetch, the local conversation context projection, current instructions, dynamic title/summary metadata, `stream: true`, and `store: false`
...
- [ ] 5.5 Implement deterministic same-conversation context projection from the current chat prompt, latest valid summary, and raw active-path tail after assistant switches
...
- [ ] 5.12 Invalidate summaries whose covered boundary is not an ancestor of the selected active leaf and rebuild from local canonical messages

## 6. Search, titles, and context summary
...
- [ ] 6.5 Implement context-summary settings for utility-assistant reference, message interval, retained raw messages, and per-Context-Profile output limits
- [ ] 6.6 Implement completed-message accounting plus foreground automatic summary after qualifying assistant responses
- [ ] 6.7 Implement incremental utility-assistant summary from a previous rolling summary and post-boundary messages, producing validated context and display summaries with budget enforcement
- [ ] 6.8 Implement active rolling summary creation with assistant/model snapshots, atomic active-summary updates, and active-path validity checks
- [ ] 6.9 Add a manual **总结上下文** action, progress indicators, retryable failures, and isolation from completed chat responses
- [ ] 6.10 Add context-limit warnings that require summary regeneration or a model/context change rather than silently dropping messages
- [ ] 6.11 Add tests proving history search ignores message bodies, summary triggers only at configured boundaries, utility output stays outside chat, and no cross-conversation data enters a summary
...
- [ ] 8.2 Implement complete local export for configuration, optional credentials, assistants, conversations, messages, summaries, settings, and binary blob entries
...
- [ ] 9.1 Add component tests for settings CRUD, assistant kinds, assistant/model switching, dynamic title/summary awareness, conversation management, message actions, search, and summary states
...
- [ ] 9.3 Add phone-viewport browser tests for first-run configuration, streaming chat, relaunch persistence, manual summary, diagnostics panel, archive/restore, and `.mobilechat` backup restore
...
- [ ] 9.7 Write README instructions for local development, GitHub Pages deployment, first-run API configuration, same-conversation memory and summary, debug diagnostics, `.mobilechat` cross-device transfer, supported protocol, and known mobile file limitations
