...
The first working implementation exposes manual **总结上下文** only in debug mode. It calls the global utility assistant referenced by the built-in context-summary feature setting, applies the active chat assistant's Context Profile, does not append visible chat messages, and updates only a small debug status hint. Automatic summary triggers may later be based on completed-turn count, idle duration, long assistant replies, or projected-context thresholds.
