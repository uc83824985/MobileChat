...
Hosted web search is serialized per protocol. For Responses, model-level web access emits `tools: [{ type: "web_search" }]`. For Chat Completions, model-level web access emits `web_search_options: {}` and relies on a search-capable route/model on the provider side. Chat Completions search has fewer controls than Responses search, so advanced controls such as live-access flags, domain filters, complete source lists, and returned-token budgets remain Responses-only until separately modeled.
